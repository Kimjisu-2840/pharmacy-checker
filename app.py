...
with st.sidebar:
    ...
    # 1. 사내 마스터 파일 직접 업로드 (롤백 완료)
    master_file = st.file_uploader("1. 사내 마스터 엑셀 업로드", type=["xlsx", "xls"])
    ...
    # 3. 명세서 파일 업로드
    order_file = st.file_uploader("3. 명세서 엑셀 업로드", type=["xlsx", "xls"])
    ...
    if st.session_state.master_db and st.session_state.order_df is not None:
        ...
        with col_left:
            ...
            with st.expander("👉 클릭하여 대량 수량 수기 증감하기", expanded=False):
                # 빈 플레이스홀더를 첫 번째 항목으로 추가 (텍스트 지우기 방지)
